...
import { Send, Sparkles, Bot, User, ListCheck, ArrowRight, CornerDownLeft, Mic, MicOff, Volume2 } from 'lucide-react';
...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  return (
    <div className="flex flex-col h-[520px] lg:h-[620px] bg-stone-900 rounded-3xl border border-stone-800 shadow-[0_10px_30px_-5px_rgba(0,0,0,0.3)] overflow-hidden relative text-stone-100">
      ...
      {/* Chat Input Bar */}
      <form onSubmit={handleSubmit} className="p-3 sm:p-4 bg-stone-950 border-t border-stone-800 flex items-center space-x-2">
        {voiceSupported && onToggleListening && (
          <button
            type="button"
            onClick={onToggleListening}
            className={`inline-flex items-center justify-center w-10 h-10 rounded-xl transition-all shadow-md shrink-0 active:scale-95 ${
              isListening
            ...
            title={isListening ? "Listening... Click to pause" : "Click to speak hands-free"}
          >
            {isListening ? (
        ...
        <input
          ...
          placeholder={isListening ? "Listening to your voice..." : "Tell the concierge about your party (e.g. '18 guests for a BBQ on Saturday, $200 budget')..."}
          className={`flex-1 px-4 py-2.5 text-xs sm:text-sm rounded-xl border focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 bg-stone-900/90 text-stone-100 placeholder:text-stone-500 transition-all ${
            isListening ? 'border-amber-500/80 bg-stone-900/95 ring-1 ring-amber-500/30' : 'border-stone-700/90'
          }`}
          disabled={isLoading}
        />
        <button
