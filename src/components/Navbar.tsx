...
import { Sparkles, Store, RotateCcw, PartyPopper, ChevronRight, SlidersHorizontal, BookOpen, Mic, MicOff, Radio } from 'lucide-react';
            ...
            <div className="flex items-center space-x-2">
              {voiceSupported && onToggleHandsFree && (
                <button
                  ...
                  className={`inline-flex items-center px-3 py-2 text-xs font-semibold rounded-xl transition-all border shadow-2xs active:scale-98 ${
                    isListening || isHandsFree
                      ? 'bg-amber-400 text-stone-950 border-amber-400 font-bold ring-2 ring-amber-400/30'
                      : 'bg-white text-stone-700 hover:bg-stone-50 border-stone-200/90'
                  }`}
                  title={isHandsFree ? "Hands-Free Voice Active - Click to stop" : "Turn on Hands-Free Voice Control"}
                >
                  {isListening ? (
                    <Radio className="w-3.5 h-3.5 mr-1.5 text-stone-950 animate-spin" />
                  ) : (
                    <Mic className="w-3.5 h-3.5 mr-1.5 text-amber-600" />
                  )}
                  <span className="hidden sm:inline">{isHandsFree ? "Voice Live" : "Hands-Free"}</span>
                </button>
              )}
